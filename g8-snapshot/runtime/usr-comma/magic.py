...
def main():
  ...
  # G8_DRM_BOOTSTRAP_V1: bind the retained continuous-splash CRTC to DSI-1
  # and a real framebuffer before any raylib client receives this DRM file.
  try:
    from g8_drm_bootstrap import bootstrap_modeset
    g8_drm_bootstrap_state = bootstrap_modeset(drm_master)
  except Exception as e:
    print(f"G8_DRM_BOOTSTRAP_V1 FAIL {type(e).__name__}: {e}", flush=True)
    raise
  try:
    os.unlink(SOCK_PATH)
  ...
  while True:
    ...
    for t in dead:
      t.join()
      clients.discard(t)
    if False and not clients and need_background:
      need_background = False
      if rl is None:
        import pyray as rl
